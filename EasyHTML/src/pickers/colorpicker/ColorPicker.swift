...
@IBDesignable internal class GradientSlider: UIView {
    ...
    private func redrawThumb() {
        thumb.frame.origin.x = CGFloat((_value - _minValue) / (_maxValue - _minValue)) * (frame.width - 30)
    }
    ...
    override internal func awakeFromNib() {
        ...
        frame.size.height = gestureHeight;
    ...
    @objc func panAction(_ sender: UIPanGestureRecognizer) {
        var newValue = (Float((sender.location(in: self).x - 15) / frame.width) * (_maxValue - _minValue) + _minValue)
        newValue = (newValue / Float(frame.width / (frame.width + 30)))
...
internal class ColorPicker: UIViewController, UIGestureRecognizerDelegate, UITextFieldDelegate, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func addDoneButtonOnNumpads() {
        ...
        inputs.forEach { input in
            input.inputAccessoryView = keypadToolbar
        }
    }
    ...
    override internal func viewDidLoad() {
        ...
        let parent = (navigationController as? ColorPickerNavigationController)

        if (parent != nil) {
            parent!.preferredContentSize = CGSize(width: 360, height: 550)
            delegate = parent!.colorPickerDelegate
            presentedModally = parent!.presentedModally
            presentedAsPopover = parent!.presentedAsPopover
        }
    ...
    internal static func getColorNameBy(red: Int, green: Int, blue: Int, alpha: CGFloat) -> String {
        ...
        if (isTransparent) {
            if (alpha == 0.0 && shouldUseKeywords) {
                return "transparent"
            }
    ...
    func updateColorName() {
        colorPreviewText.text = ColorPicker.getColorNameBy(red: Int(red * 255), green: Int(green * 255), blue: Int(blue * 255), alpha: alpha)
    }
        ...
        DispatchQueue(label: "easyhtml.colorpicker.animationqueue").async {
            ...
            for i in range {
                ...
                if (currentSleepTime > 0) {
                    usleep(UInt32(currentSleepTime))
                }
    ...
    func inputUpdatedWith(id: Int, value: Float, animated: Bool = false) {
        ...
        if (id == 0) {
            if (intValue == Int(red * 255)) {
                return
            };
            red = CGFloat(value / 255.0)
        } else if (id == 1) {
            if (intValue == Int(green * 255)) {
                return
            };
            green = CGFloat(value / 255.0)
        } else {
            if (intValue == Int(blue * 255)) {
                return
            };
    ...
    override internal func viewDidLayoutSubviews() {
        ...
        colorPickerCursor.frame.origin.y = saturation * colorPickerImage.frame.height - 10
        brightnessPickerCursor.frame.origin.y = (1 - brightness) * (brightnessPickerGradientView.frame.height - 10)
        alphaPickerCursor.frame.origin.y = (1 - alpha) * (alphaPickerImage.frame.size.height - 10)
    }
