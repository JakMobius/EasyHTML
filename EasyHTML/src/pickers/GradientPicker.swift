...
internal class TCGradient: NSObject, NSCoding, NSCopying {
    internal required init?(coder aDecoder: NSCoder) {
        self.colors = aDecoder.decodeObject(forKey: "1") as! [UIColor]
        self.location = aDecoder.decodeObject(forKey: "2") as! [NSNumber]
        self.startPoint = aDecoder.decodeCGPoint(forKey: "3")
        self.endPoint = aDecoder.decodeCGPoint(forKey: "4")
        self.isRadial = aDecoder.decodeBool(forKey: "5")
        self.angle = aDecoder.decodeFloat(forKey: "6")
    }
...
class GradientLayer: CALayer {
    ...
    internal var gradientOptions: CGGradientDrawingOptions {
        get {return _gradientOptions}
        set {_gradientOptions = newValue; setNeedsDisplay()}
    }
    internal var isRadial: Bool {
        get {return _isRadial}
        set {_isRadial = newValue; setNeedsDisplay()}
    }
    internal var startPoint: CGPoint {
        get {return _startPoint}
        set {_startPoint = newValue; setNeedsDisplay()}
    }
    internal var endPoint: CGPoint {
        get {return _endPoint}
        set {_endPoint = newValue; setNeedsDisplay()}
    }
    internal var locations: [CGFloat] {
        get {return _locations}
        set {_locations = newValue; setNeedsDisplay()}
    }
    internal var colors: [CGColor] {
        get {return _colors}
        set {_colors = newValue; setNeedsDisplay()}
    }
    ...
    override func draw(in ctx: CGContext) {
        ...
        if(isRadial){
            ...
            {
                let realStartPoint = CGPoint(x: startPoint.x * self.frame.width, y: startPoint.y * self.frame.height)
                let realEndPoint = CGPoint(x: endPoint.x * self.frame.width, y: endPoint.y * self.frame.height)
...
internal class GradientPicker: UIViewController, UITableViewDataSource, UITabBarDelegate, UITextFieldDelegate, UITableViewDelegate, ColorPickerDelegate, UICollectionViewDelegate, UICollectionViewDataSource, UIGestureRecognizerDelegate, NotificationHandler {
    ...
    // #TODO: доработать механизм чтения
    ...
    override internal func viewDidLoad() {
        ...
        let nc = self.navigationController as! GradientPickerNavigationController
        self.prefix = nc.gradientPrefix
        self.unsupportedRadialGradientOptions = nc.radialGradientUnsupportedOptions
        self.delegate = nc.gradientDelegate
        ...
        if((self.navigationController as! GradientPickerNavigationController).presentedAsPopover){
        ...
        let rotationgesture = UIRotationGestureRecognizer(target: self, action: #selector(rotateGradientByGesture))
        
        rotationgesture.delegate = self
        
        gradientPreview.addGestureRecognizer(rotationgesture)
    ...
    internal static func positionsFromAngle(degree: CGFloat, mult: CGFloat = 1.0) -> [CGPoint] {
        ...
        let multiplicator: CGFloat = sqrt(length + 1) * mult
        
        let startPoint = CGPoint(x: mult - sine * multiplicator, y: mult - cosine * multiplicator)
        let endPoint = CGPoint(x: mult + sine * multiplicator, y: mult + cosine * multiplicator)
    ...
    internal func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return section == 0 ? currentGradient.colors.count : 1
    }
    ...
    private var translate: CGFloat = 0 {
        didSet {
            self.view.transform.ty = -translate
        }
    }
    ...
    internal override func viewWillDisappear(_ animated: Bool) {
        if(gradientPickerDidConfirm) { return }
    ...
    func editColor(for view: UIView, initialColor: UIColor? = nil) {
        ...
        ColorPicker.present(inNavigationViewController: self.navigationController!)
    ...
    @objc func slideToRecentGestureRecogniser(_ sender: UIPanGestureRecognizer) {
        ...
        if sender.state == .began {
            ...
            if !slideConfirmed {
                ...
                self.recentContainer.isHidden = false
    ...
    internal func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return GradientPicker.recentGradientsList.count
    }
    ...
    internal func setGradient(newGradient: TCGradient) {
        ...
        if(needsChangeRadialState) {
            self.segmentedControl.selectedSegmentIndex = newGradient.isRadial ? 1 : 0
