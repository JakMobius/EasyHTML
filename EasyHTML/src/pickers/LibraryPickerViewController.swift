...
internal class Library: NSObject {
    /// Название библиотеки
    internal var name: String
    /// Расположение библиотеки в сети
    internal var networkURL: String
    /// Загружена ли библиотека на устройство
    internal var isLoaded: Bool
    /// Идёт ли загрузка библиотеки в данный момент
    internal var isLoading: Bool = false
    /// Название раздела, содержащего эту библиотеку
    internal var section: LibrarySection
    /// Расширение файла библиотеки вместе с точкой
    ...
    /// Получить локальный путь к библиотеке
    internal func getLocalFileURL() -> URL {
        return URL(fileURLWithPath: applicationPath + "/libraries/").appendingPathComponent(section.name).appendingPathComponent(name + ext)
    }
}
...
internal class LibraryPickerViewController: AlternatingColorTableView, UISearchResultsUpdating {
    ...
    override internal func viewDidLoad() {
        ...
        self.navigationItem.leftBarButtonItem = UIBarButtonItem(title: localize("cancel"), style: .plain, target: self, action: #selector(close))
        ...
        DispatchQueue(label: "Library list reader queue").async {
            ...
            guard let url = Bundle.main.url(forResource: self.libraryType.rawValue, withExtension: "json") else {return}
            guard let jsondata = try? JSONSerialization.jsonObject(with: Data(contentsOf: url), options: []) else {print("Bad JSON at url \(url). Aborting."); return}
            guard let array = jsondata as? Array<Array<Any>> else {print("[EasyHTML] Wrong JSON format at url \(url). Aborting.");return}
            
            let savedLibraryesUrlString = applicationPath + "/libraries/"
            let savedLibraryesUrl = URL(fileURLWithPath: savedLibraryesUrlString)
            
            if(!isDir(fileName: savedLibraryesUrlString)) {
                try? FileManager.default.createDirectory(atPath: savedLibraryesUrlString, withIntermediateDirectories: false, attributes: nil)
            }
            
            array.forEach {
                item in
                guard item.count == 2 else {return}
                guard let sectionName = item[0] as? String else {return}
                guard let libraryList = item[1] as? [[String]] else {return}
                let sectionUrl = savedLibraryesUrl.appendingPathComponent(sectionName)
                ...
                libraryList.forEach {
                    library in
                    guard library.count == 2 else {return}
    ...
    override internal func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        return isFiltering() ? localize("findresults").replacingOccurrences(of: "#", with: "\(filteredLibraries.count)") : librarySections[section].name
    }
    ...
    internal override func numberOfSections(in tableView: UITableView) -> Int {
        return isFiltering() ? 1 : librarySections.count
    }

    internal override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return isFiltering() ? filteredLibraries.count : librarySections[section].libraries.count
    }
    
    internal override func tableView(_ tableView: UITableView, canEditRowAt indexPath: IndexPath) -> Bool {
        return getCurrentLibrary(for: indexPath).isLoaded
    }
