...
class BootUtils {
    @objc func keyboardAppearance() -> UIKeyboardAppearance {
        return (userPreferences.adjustKeyboardAppearance && userPreferences.currentTheme.isDark) ? .dark : .light
    }
    ...
    static func checkSharedFolderExistance() -> URL! {
    ...
    static func deleteSharedFolderIfExist() {
        if let url = checkSharedFolderExistance() {
