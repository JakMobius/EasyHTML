...
class FTPNewServerDialogTableViewController: AlternatingColorTableView, UITextFieldDelegate {
    ...
    internal var serverIsValid: Bool {
        get {
            nameValid && hostValid && portValid && (loginTypeIsAnonymous || loginValid)
        }
    }
    ...
    @objc internal func nameChanged() {
        ...
        guard let text = cell.input.text else {
            return
        }
    ...
    @objc internal func hostChanged() {
        ...
        guard let cell = hostCell else {
            return
        }
        guard var text = cell.input.text else {
            return
        }
        guard !text.isEmpty else {
            return
    ...
    @objc internal func portChanged() {
        ...
        guard let cell = portCell else {
            return
        }
        guard let text = cell.input.text, !text.isEmpty else {
            portValid = true; return
        }
        guard let port = Int(text) else {
            return
        }
        guard port > 0 else {
            return
    ...
    @objc internal func usernameChanged() {
        ...
        guard let cell = usernameCell else {
            return
        }
        guard cell.input.hasText else {
            return
        }
