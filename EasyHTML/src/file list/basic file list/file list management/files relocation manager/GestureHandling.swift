...
extension FilesRelocationManager {
    @objc internal func gestureRecognizerAction(_ sender: UIPanGestureRecognizer) {
        ...
        let previousTouchLocation = previousTouchLocation
            ...
            if isMovingVertical {
                ...
                if location.y < -10 {
                    ...
                    if draggingIsLocked == true {
                        translation = bounceModify(translation: location.y + 10, coefficient: 3)
                    } else {
                        translation = bounceModify(translation: location.y + 10, coefficient: 30)
                    }

                    if draggedFile == nil {
                        draggedFile = replicationViewsContainer.hitTest(sender.location(in: replicationViewsContainer), with: nil)
                        while draggedFile != nil && !(draggedFile is ReplicationView) {
                            draggedFile = draggedFile.superview

                            if draggedFile == replicationViewsContainer {
                                draggedFile = nil
                            }
                        }

                        if let draggingFile = draggedFile {
                        ...
                        draggedFile?.superview?.bringSubviewToFront(draggedFile)
                    }
                    if let draggedFile = draggedFile {

                        draggedFile.transform.ty = 0
                        draggedFile.frame.origin.y = 5
                        draggedFile.transform.ty = translation
                        layout()
                    }
                } else {
                    if draggingIsLocked {
                    ...
                    if location.y > 10 {
                        ...
                        if translation < 60 {
                            translation = bounceModify(translation: translation, coefficient: 6)
                        } else {
                            translation = 60 - bounceModify(translation: 60 - translation, coefficient: 6)
                        }
    ...
    final func handleHorizontalGesture(location: CGPoint) {
        if (location.x < -10) {
            ...
            if location.x > 10 {
                ...
                if filesToMove.count == 1 {
                    replicationViewsContainer.transform = CGAffineTransform(translationX: bounceModify(translation: location.x - 10, coefficient: 30), y: 0)
                    return
                }
                ...
                if percentage > 1 {
                    percentage = bounceModify(translation: percentage - 1, coefficient: 2) + 1
                }
    ...
    internal func handleVerticalGesture(location: CGPoint) {
        ...
        let coefficient = width / height

        var yTranslation = location.y

        if yTranslation > 0 {
            yTranslation = bounceModify(translation: yTranslation, coefficient: 10)
        } else if filesToMove.count == 1 {
            yTranslation = bounceModify(translation: yTranslation, coefficient: 30)
        } else {
            ...
            if !draggingIsLocked {
                ...
                if top < maxTop {
                    ...
                    yTranslation -= bounceModify(translation: overflow, coefficient: 50)
                }
            } else {
                yTranslation = bounceModify(translation: yTranslation, coefficient: 3)
            }

            replicationViewsContainer.transform = CGAffineTransform(translationX: xTranslation, y: yTranslation)

            var size = -yTranslation * coefficient + 20
