...
internal class FilesRelocationManager: NSObject, UIGestureRecognizerDelegate, UIScrollViewDelegate {
    ...
    var draggedFile: UIView!
    var sourceType: FileSourceType!
    var sourceContainer: SharedFileContainer!
    var previousTouchLocation: CGPoint! = nil

    /**
     Indicating whether file relocation is locked now
     - warning: Use only `lockDragging` and `unlockDragging` to modify this field
     */
    ...
    internal func addFile(file: FSNode) {
        ...
        replicationView.setEditing(isEditing)
        replicationView.alpha = 0.0
    ...
    internal init(cells: [UITableViewCell]!, files: [FSNode], parent: UINavigationController, source: SharedFileContainer) {
        ...
        sourceDirectory = source.url
        replicationViews = []
        filesToMove = files
        self.parent = parent
        replicationViewsContainer = CustomScrollView()
        currentLayout = .normal(size: 20)
        sourceContainer = source
        sourceType = sourceContainer.sourceType
    ...
    final func layout(force: Bool = false) {
        ...
        case .horizontal:
            ...
            for i in startIndex...endIndex {
                if let draggingfile = draggedFile, draggingfile.tag == i {
    ...
    internal func stopRelocatingFiles() {
        ...
        tableview?.tableView.reloadData() // TODO: Rewrite
    }

    final func bounceModify(translation: CGFloat, coefficient: CGFloat = 100) -> CGFloat {
        let translation = translation / coefficient
        var result: CGFloat
    ...
    // Restores tagging order of cards
    ...
    final func animateCardFlight(index: Int) {
        ...
        // The code below is responsible for the animation and preparation for it.

        let position = view.superview!.convert(view.center, to: parent.view)
    ...
    final func fileCopied(file: FSNode) {
        ...
        if replicationViews.isEmpty {
            stopRelocatingFiles()
        } else if replicationViews.count == 1, case .horizontal = currentLayout {
            currentLayout = .normal(size: 20)
        }
    }
        ...
        if let destination = parent.topViewController as? SharedFileContainer {
            ...
            if let source = sourceContainer {
                ...
                func relocateNextFile() {
                    guard let file = iterator.next() else {
                        return
                    }
                    oldProgress = 0;
                    ...
                    func tryRelocateFile() {
                        ...
                        let receiveability = destination.canReceiveFile(file: file, from: sourceType)
