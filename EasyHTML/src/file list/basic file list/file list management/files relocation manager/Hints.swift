...
extension FilesRelocationManager {
    enum HintType {
        ...
        public var key: String {
            self == .normal ? "movehint" : "movehinthorizontal"
        }
        ...
        public func localizedTextForIteration(_ iteration: Int) -> String {
            localize(/*# -tcanalyzerignore #*/"\(key)\(iteration)", .files)
        }
    }

    final var shouldPreviewGuide: Bool {
        get {
            Defaults.bool(forKey: "fileMovingGuideShouldPreview", def: true)
        }
        set {
    ...
    final var shouldPreviewHorizontalGuide: Bool {
        get {
            Defaults.bool(forKey: "fileMovingHorizontalGuideShouldPreview", def: true)
        }
        set {
    ...
    @objc func updateGuide() {
        ...
        if let sign = currentSign {
            ...
            currentSign?.label.setTextWithFadeAnimation(text: hintType.localizedTextForIteration(i), duration: 0.5, completion: nil)
    ...
    final func restoreGuideSigns() {
        guard currentSign == nil || currentSign!.tag == -1 else {
            return
        }
