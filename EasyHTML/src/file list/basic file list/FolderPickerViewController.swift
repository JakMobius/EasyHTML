    ...
    internal class Cell: UITableViewCell {
        internal override func layoutSubviews() {
            ...
            if(self.imageView!.image != nil) {
                self.imageView?.frame = CGRect(x: 16,y: self.frame.height / 2 - 16,width: 32,height: 32)
                self.textLabel!.frame.size.width = self.contentView.frame.width - 70
                self.textLabel!.frame.origin.x = 64
            }
        }
    }
