...
internal class FileListController: BasicMasterController, UIGestureRecognizerDelegate, UITextViewDelegate, SharedFileContainer {
    ...
    public var isCurrentViewController: Bool {
        if let nc = self.navigationController, nc.viewControllers.last != self {
            return false
        }
    ...
    /**
     Вызывается, когда контроллер загружен в пямять. перезаписывая этот
     метод можно совершить дополнительную настройку контроллера
     до его появления на экране.
     
     - warning: При перезаписывании этого метода обязателен вызов `super.viewDidLoad()` для корректной работы `FileListController`
     */
    ...
    // Оптимизировать обработку изменений: использовать лишь имена файлов
    // вместо того, что бы сравнивать полные пути (Cделано). Ну, и использовать
    // несколько процессоров для обработки, если получится. Может
    // еще что-нибудь для оптимизации придумаю. #TODO
    ...
    internal func updateFileListAnimated(old: [FSNode]) {
        ...
        for (i, file) in old.enumerated() {
            ...
            if !newFiles.contains(where: { nfile -> Bool in return nfile.name == file.name }) {
    ...
    internal var emptyFolderWarningShown: Bool {
        get {
            return emptyFolderTopLabel != nil
        }
    }
    ...
    internal func hideEmptyFolderWarning() {
        ...
        guard let topLabel = self.emptyFolderTopLabel else {return}
        self.emptyFolderTopLabel = nil
        
        guard let bottomLabel = self.emptyFolderBottomLabel else {return}
        self.emptyFolderBottomLabel = nil
    ...
    internal override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return _isLoading ? 27 : fileListDataSource?.countOfFiles() ?? 0
    }
    
    private func nativePreviewImageFor(_ file: FSNode) -> UIImage {
        if file is FSNode.File {
            return getFilePreviewImageByExtension(file.url.pathExtension, inversed: userPreferences.currentTheme.isDark)
        } else {
            if userPreferences.currentTheme.isDark {
                return FilePreviewImages.folderImageInversed
            } else {
    ...
    internal func lightPreviewImageFor(file: FSNode, at index: Int) -> UIImage! {
        return previewImageFor(file: file, at: index)
    }
    ...
    func fileLongTouched(at index: Int) {
        ...
        guard let actions = fileListDataSource.shortcutActionsForFile?(file: file, at: index) else { return }
        ...
        _loadingAnimationTask = DispatchWorkItem(block: { [weak self] in
            ...
            while let slf = self, slf._isLoading {
                DispatchQueue.main.sync {
                    guard slf.parent != nil else { return }
                    guard let indexPaths = slf.tableView.indexPathsForVisibleRows else {return}
                    
                    for indexPath in indexPaths {
                        let row = Float(indexPath.row)
                        
                        guard let cell = slf.tableView.cellForRow(at: indexPath) as? FileListPlaceholderCell else {return}
    ...
    /**
     Переменная, отображающая экран загрузки при значении `true`.
     После изменения вызвать `tableView.reloadData()`
     */
    ...
    internal override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if(isSelectingFilesToMove) {
            ...
            /*
             Следующие 8 строк кода отвечают за удаление забаганного
             separatorView, который появляется ни с того ни с сего
             Возможно, позже я найду более элегантное и правильное
             решение, но пока что так.
             */
            ...
            // Что, пересчитал, да? Убедился, что здесь правда 8 строк? Молодец.
    ...
    @objc internal func fileForceTouched(_ sender: ForceTouchGestureRecognizer) {
        ...
        if(sender.state == .cancelled || self.presentedViewController != nil) {
            return
        }
    ...
    func showFileSizeWarning(callback: @escaping () -> ()) {
        ...
        PrimarySplitViewController.instance(for: self.view).present(alert, animated: false)
    }
    ...
    // MARK: Работа с интерфейсом выбора файлов для перемещения
    ...
    override func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        // Переопределение метода в AlternatingColorTableView
    }
    ...
    /*
     Не спрашивай, как это работает. Это работает. И черт с ним.
     Если найдешь в этом куске не пойми чего какую-то ошибку, то...
     покойся с миром.
     */
    
    // MARK: Работа с менеджером перемещения файлов
