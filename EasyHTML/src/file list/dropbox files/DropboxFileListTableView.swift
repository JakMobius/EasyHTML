...
internal class DropboxFileListTableView: FileListController, FileListDelegate, FileListDataSource, UIPopoverPresentationControllerDelegate, FileDetailDelegate, FileDetailDataSource, FileCreationDialogDelegate, NewFileDialogDelegate, LibraryPickerDelegate {
    ...
    internal func startLongpoolRequest() {
        ...
        func startNextRequest(delay: TimeInterval) {
            waitingForNextLongpoolRequest = true
            ...
            if let result = result {
                ...
                result.entries.forEach({ (metadata) in
                    ...
                    if let metadata = metadata as? Files.DeletedMetadata {
                        ...
                        // The index(of: FSObject) is not used here, because the comparison of Dropbox
                        // files goes by their IDs, which would interfere with the animation of renaming
                        // a file, since it is is implemented by "moving" it within the same folder, but
                        // with different When you move a file, the longpool-request first sends a signal
                        // to remove file from the source directory, but the file ID does not change.
                        ...
                        TemporaryFileMetadataManager.clearMetadata(forFile: deletedFile)
                        ...
                        if let metadata = metadata as? Files.FileMetadata {
                            ...
                            // We don't create a new File object here to save time on
                            ...
                            // We don't create a new Folder object here to save time on
                            // object instantiation and keep information about folder
                            // contents.
    ...
    internal func reloadDirectory() {
        ...
        // Dropbox API fails when trying to fetch root directory contents with path = "/"
    ...
    internal func navigateTo(directory: FSNode.Folder) {
        ...
        // Cache controllers to reduce allocation overhead
    ...
    override internal func tableView(_ tableView: UITableView, accessoryButtonTappedForRowWith indexPath: IndexPath) {
        ...
        let detailController = FileDetailViewController.getNew(observingFile: files[indexPath.row], actions: [.delete, .rename, .clone, .move])
        ...
        if file is FSNode.Folder {
            ...
            if scheme == nil {
                actions.append(.showSourceAction {
                    _ in
                    self.openSourceCode(file: file, config: [.ioManager: DropboxIOManager()])
                })
            } else {
                actions.append(.showContentAction {
                    _ in
                    self.openInBrowser(file: file, config: [.ioManager: DropboxIOManager()], force: true)
                })
            }
    ...
    /*
     internal func fileDetail(creationDateOf file: FSObject) -> Date? {
        // The file creation date cannot be obtained in the current version of the Dropbox API.
        // Forced return of nil in this method is equivalent to rejection of its implementation
        // in classes that implement the FileDetailDataSource protocol. To avoid wasting memory
        // on an empty method, it is left unimplemented.
     
        // return nil // <-- Uncomment in case of nuclear war
    }
     */
    ...
    func newFileDialog(dialog: NewFileDialog, hasPicked image: UIImage) {
        ...
        func startRequest() {
            ...
            let path = url!.appendingPathComponent(getAvailableFileName(fileName: "image.png")).path
    ...
    private func uploadLibrary(library: Library) {
        ...
        guard let data = try? Data(contentsOf: localURL) else {
            print("[EasyHTML] [DropboxFileListTableView] -libraryPicker(didSelect library:Library) Failed to retrieve library binary data! Cancelling.")
            return
        }
    ...
    private func uploadFolder(at localURL: URL, to destinationURL: URL, callback: @escaping (Error?) -> (), progress: ((Float) -> ())?) {
        ...
        // Copy an entire folder file-by-file to the Dropbox server
