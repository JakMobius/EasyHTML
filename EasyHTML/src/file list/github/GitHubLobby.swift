...
internal class GitHubLobby: BasicMasterController {
    ...
    @objc private func searchFieldDidEnter(field: UITextField) {
        guard var text = field.text else { return }
...
//    }
    ...
    override func tableView(_ tableView: UITableView, didEndDisplaying cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        // Не реализован в суперклассе, вызов приведет к крэшу
    ...
    internal override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        if searching {
            if section == 1 {
                // Если мы должны показывать активити-индикатор, в первой
                // строчке должна быть только одна строчка.
                ...
                if searchTask == nil {
                    // Крайний случай - отображение строчки "Нет репозиториев"
                    
                    if searchResult.totalRepoCount == 0 {
                        return 1
                    }
                    
                    // Другой крайний случай - отображение дополнительной
                    // строчки "Еще x репозиториев"
                    ...
                    // На момент, когда я решил это раскомментировать,
                    // я забыл наглухо когда используется этот случай.
                    // Но оно точно где-то используется
                    
                    return 1
                }
            } else {
                
                // Если мы должны показывать активити-индикатор в первой секции
                // во второй секции не должно быть строк.
        ...
        if searching {
            if indexPath.section == 1 {
                ...
                if indexPath.row == searchResult.repositoriesFound.count {
                    ...
                    guard let name = repo.name else { return }
            ...
            if indexPath.section == 1 {
                ...
                case .searched(let request):
                    guard searchCell != nil else { return  }/*  Этого точно никогда не произойдет,
                                                                но на всякий случай... */
