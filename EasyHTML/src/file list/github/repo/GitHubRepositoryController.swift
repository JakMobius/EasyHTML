...
class GitHubRepositoryController: GitHubPreviewController, SharedFileContainer {
    ...
    func canReceiveFile(file: FSNode, from source: FileSourceType) -> FilesRelocationManager.FileReceiveAbility {
        .no(reason: .unsupportedController)
    }
    ...
    var sourceType: FileSourceType {
        .github(repo: repositoryFullName, commit: commitName)
    }
    ...
    private var commitName: String {
        get {
            commitID ?? fetchedRepositoryData.defaultBranch ?? "master"
        }
    }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if indexPath.section == 1 {
            if indexPath.row == 0 {
                ...
                controller.commitID = commitName
                controller.repositoryFullName = repositoryFullName
    ...
    func fetchInfo() {
        ...
        queryRequest = GitHubAPI.enqueueQuery(query: query, arguments: [
    ...
    override func viewDidLoad() {
        ...
        repositoryOwner = String(fractions[0])
        repositoryName = String(fractions[1])
    ...
    func getURL() -> URL? {
        URL(string: "https://github.com/" + repositoryFullName)
    }
    ...
    func cloneRepo(action: UIAlertAction) {
        let file = GitHubFile(url: URL(string: "/" + repositoryName + ".zip")!, sourceType: sourceType, sha: "")
    ...
    override func viewDidLayoutSubviews() {
        ...
        guard let cell = readMeCell else {
            return
        }

        // Cell width is updated only on the end of the event loop
