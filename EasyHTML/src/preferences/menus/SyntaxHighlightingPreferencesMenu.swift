...
struct SyntaxHighlightScheme {
    ...
    func serialize() -> SerializedType {
        [ext, mode.cmMimeType, mode.configurationFiles, mode.description]
    }
    ...
    static func deserialize(type: SerializedType) -> SyntaxHighlightScheme? {
        ...
        guard let ext = type[0] as? String else {
            return nil
        }
        guard let mime = type[1] as? String else {
            return nil
        }
        guard let files = type[2] as? [String] else {
            return nil
        }
        guard let desc = type[3] as? String else {
            return nil
        }
...
class SyntaxHighlightingPreferencesMenu: AlternatingColorTableView, SyntaxHighlightingPickerDelegate, SyntaxHighlightSchemeCreationDialogDelegate {
    ...
    override func numberOfSections(in tableView: UITableView) -> Int {
        userPreferences.syntaxHighlightingEnabled ? 2 : 1
    }

    override func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        section == 1 ? localize("codehighlightingconf", .preferences) : nil
    }
