...
internal class ArchivingOptions: NSObject {
...
@objc internal protocol FileDetailDataSource {
    ...
    /**
        A method that is called when the "Share" button is tapped
     
     - parameter controller: Controller to display `UIActivityController` on, if it's necessary to pre-download the file
     - parameter file: The file to be sent. The value is identical to `controller.file`.
     - parameter callback: The function to be called at the end of caching. The first argument is a flag indicating
        whether the file should be deleted after being sent, in case it was cached in a temporary folder. The second
        argument should contain the path to the cached file. If there was an error during the caching process, the
...
internal class FileDetailViewController: AlternatingColorTableView, UITextFieldDelegate, ArchiveDialogDelegate {
    ...
    internal static func getNew(observingFile: FSNode, actions: [Action]) -> FileDetailViewController {
    ...
    @objc func shareFile() {
        ...
        dataSource?.fileDetail?(controller: self, objectsToShare: file, callback: {
            shouldDelete, url in
    ...
    private func readAttributes() {
        ...
        if let request = dataSource?.fileDetail(sizeOf:completion:) {
    ...
    private func setupNavigationBar() {
        navigationItem.title = isFolder ? localize("aboutfolder") : localize("aboutfile")
