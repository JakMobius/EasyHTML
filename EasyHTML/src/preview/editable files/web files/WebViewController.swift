...
class WebViewController: UIViewController, WKScriptMessageHandler, WKUIDelegate, WKURLSchemeHandler, NotificationHandler {
    ...
    private func handleLoadingError(error: Error?) {
        ...
        self.messageManager.newWarning(message: localize("downloadknownerror") + "\n" + message)
            ...
            .withCloseable(false)
            .withButton(EditorWarning.Button(title: localize("tryagain"), target: self, action: #selector(self.reload)))
            .present()
    ...
    override func viewDidLoad() {
        ...
        if #available(iOS 11.0, *) {
            config.setURLSchemeHandler(WKURLSchemeHandlerLeakAvoider(delegate: self), forURLScheme: customURLScheme)
        }
    ...
    @objc func reload() {
        ...
        //let tabbarcontroller = (self.tabBarController as! WebEditorController)
        //tabbarcontroller.isExecuteButtonEnabled = false
    ...
    func parseJavaScriptAttributedString(origin: String, colorInfo: String) -> NSMutableAttributedString {
        ...
        for i in stride(from: 0, to: items.count - 1, by: 2) {
            ...
            if(color == -1 && !cursive) {
                location += length
                continue // NSAttributedString жрёт так много памяти, что просто охренеть.
                         // поэтому как можно меньше добавляем аргументов в это дело.
            }
    ...
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        case WebViewActions.consoleMessageAction:
            ...
            guard let body = message.body as? [Any] else {return}
            if(body.count != 3) {return}
            guard let message = body[0] as? String else {return}
        ...
        case WebViewActions.didFinishLoadAction:
            ...
            if(dispatcher.isScript && !dispatcher.isWebpage)
            {
                dispatcher.getContent {
                    response, _ in
                    guard let content = response as? String else { return }
