...
/// Button which automatically sets its color to buttonDarkColor
...
internal class FindReplaceDialogMessage: UIImageView {
    ...
    internal init(parent: UIView, type: MessageActionType) {
        ...
        backgroundColor = userPreferences.currentTheme.cellSelectedColor;
        layer.cornerRadius = 10.0

        image = self.type == .goToEnd ? #imageLiteral(resourceName: "jump-to-end").withRenderingMode(.alwaysTemplate) : #imageLiteral(resourceName: "jump-to-start").withRenderingMode(.alwaysTemplate)
        contentMode = .center
        contentScaleFactor = 4.0
        tintColor = userPreferences.currentTheme.secondaryTextColor

        parent.centerXAnchor.constraint(equalTo: centerXAnchor).isActive = true
        parent.centerYAnchor.constraint(equalTo: centerYAnchor).isActive = true
    ...
    internal func present() {
        ...
        transform = CGAffineTransform(translationX: 0.8, y: 0.8)
...
class FindReplaceDialogContainerView: UIView, UITextFieldDelegate, NotificationHandler {
    ...
    private var previousOccurrenceButton: DarkButton!
    private var nextOccurrenceButton: DarkButton!
    ...
    override func layoutSubviews() {
        if let splitButtonConstraint = splitButtonConstraint, let splitButtonSecondaryConstraint = splitButtonSecondaryConstraint {
            if (frame.width < 500 && !replaceDialogWillHideSoon) {
    ...
    internal init(parent: UIView, type: ButtonActionType, webView: WKWebView) {
        ...
        parent.addSubview(self)
        translatesAutoresizingMaskIntoConstraints = false
        parent.leftAnchor.constraint(equalTo: leftAnchor).isActive = true
        parent.rightAnchor.constraint(equalTo: rightAnchor).isActive = true
        parent.topAnchor.constraint(equalTo: topAnchor).isActive = true
        parent.bottomAnchor.constraint(equalTo: bottomAnchor).isActive = true
        setupThemeChangedNotificationHandling()
    }
        ...
        if animated {
            ...
            UIView.animate(withDuration: 1.0, delay: 0.0, usingSpringWithDamping: 0.4, initialSpringVelocity: 0.0, options: [], animations: {
                ...
                if self.searchingStarted {
                    ...
                    self.previousOccurrenceButton?.removeFromSuperview()
                    self.nextOccurrenceButton?.removeFromSuperview()
                } else {
            ...
            UIView.animate(withDuration: 1.0, delay: 0.0, usingSpringWithDamping: 0.4, initialSpringVelocity: 0.0, options: [], animations: {
                ...
                if self.searchingStarted {
                    ...
                    fade(self.previousOccurrenceButton)
                    fade(self.nextOccurrenceButton)
            ...
            sourceTappedButton.addSubview(sourceTappedButton.imageView!)
            sourceTappedButton.isHidden = false
            closeButtonView?.removeFromSuperview()
            fakeButton.removeFromSuperview()
            replaceButton?.removeFromSuperview()
            replaceAllButton?.removeFromSuperview()
            searchField.removeFromSuperview()
            replaceField?.removeFromSuperview()
            previousOccurrenceButton?.removeFromSuperview()
            nextOccurrenceButton?.removeFromSuperview()
    ...
    internal func updateSearchResults() {
        guard !replaceDialogWillHideSoon else {
            return
        }
        webView.evaluateJavaScript("""
                                       (function(){
    ...
    @objc private func replaceAll() {
        ...
        if isCompact {
            matchesFoundView.transform = CGAffineTransform(translationX: 0, y: 30)
            matchesFoundView.isHidden = false
            matchesFoundView.alpha = 0
            fakeButton.isHidden = false
            fakeButton.transform = CGAffineTransform(translationX: -50, y: 0)
        }
        ...
        UIView.animate(withDuration: 0.5) {
            ...
            hide(self.previousOccurrenceButton)
            hide(self.nextOccurrenceButton)
        ...
        matchesFoundView.text = localize("replacing", .editor)
    ...
    @objc func focus() {
        ...
        let step = step
        let searchingStarted = searchingStarted
        ...
        UIView.animate(withDuration: 0.3, delay: 0, options: [], animations: {
            ...
            if searchingStarted {
                ...
                self.nextOccurrenceButton?.alpha = 0
                self.previousOccurrenceButton?.alpha = 0
            ...
            if searchingStarted {
                ...
                self.nextOccurrenceButton?.isHidden = true
                self.previousOccurrenceButton?.isHidden = true
    ...
    func search() {
        ...
        if (previousOccurrenceButton == nil) {
            ...
            previousOccurrenceButton = setupButton(rotation: 1.57, action: #selector(findPrevious))
            nextOccurrenceButton = setupButton(rotation: 4.71, action: #selector(findNext))

            previousOccurrenceButton.addTarget(self, action: #selector(findPrevious), for: .touchUpInside)
            nextOccurrenceButton.addTarget(self, action: #selector(findNext), for: .touchUpInside)

            nextOccurrenceButton.rightAnchor.constraint(equalTo: rightAnchor, constant: -40.0).isActive = true
            previousOccurrenceButton.rightAnchor.constraint(equalTo: nextOccurrenceButton.leftAnchor, constant: -5.0).isActive = true
            nextOccurrenceButton.widthAnchor.constraint(equalToConstant: 30).isActive = true
            previousOccurrenceButton.widthAnchor.constraint(equalToConstant: 30).isActive = true
            ...
            if (type == .replace) {
                ...
                splitButtonConstraint = replaceAllButton.rightAnchor.constraint(equalTo: previousOccurrenceButton.leftAnchor, constant: -10.0)
                splitButtonSecondaryConstraint = replaceButton.leftAnchor.constraint(equalTo: leftAnchor, constant: 10.0)
                splitButtonSecondaryConstraint.isActive = false
            ...
            layout(previousOccurrenceButton)
            layout(nextOccurrenceButton)
        } else {
            previousOccurrenceButton?.isHidden = false
            nextOccurrenceButton?.isHidden = false
        ...
        fadeIn(previousOccurrenceButton)
        fadeIn(nextOccurrenceButton)
    ...
    internal func present(tapedButton button: UIButton, animated: Bool) {
        ...
        searchField.rightAnchor.constraint(equalTo: rightAnchor, constant: -50.0).isActive = true
        searchField.topAnchor.constraint(equalTo: topAnchor).isActive = true
        searchField.bottomAnchor.constraint(equalTo: bottomAnchor).isActive = true
        searchField.leftAnchor.constraint(equalTo: leftAnchor, constant: 50.0).isActive = true
        ...
        closeButtonView.rightAnchor.constraint(equalTo: rightAnchor).isActive = true
        closeButtonView.topAnchor.constraint(equalTo: topAnchor).isActive = true
        closeButtonView.bottomAnchor.constraint(equalTo: bottomAnchor).isActive = true
        ...
        if animated {
            ...
            fakeButton.frame = CGRect(x: 2, y: 0, width: 40, height: 40)
        }
    }
    ...
    func nextStep() {
        ...
        if (step != 1 && searchField.text!.isEmpty) {
            return
        }
        ...
        if (type == .search) {
            ...
            if (replaceField == nil) {
                ...
                replaceField.rightAnchor.constraint(equalTo: rightAnchor, constant: -50.0).isActive = true
                replaceField.topAnchor.constraint(equalTo: topAnchor).isActive = true
                replaceField.bottomAnchor.constraint(equalTo: bottomAnchor).isActive = true
                replaceField.leftAnchor.constraint(equalTo: leftAnchor, constant: 50.0).isActive = true
