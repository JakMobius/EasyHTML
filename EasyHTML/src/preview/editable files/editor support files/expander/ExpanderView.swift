...
class ExpanderView: UIView, NotificationHandler {
    ...
    var holdedButtons: [Int] = []
    
    @objc func buttonDown(_ sender: ExpanderButton) {
        guard !holdedButtons.contains(sender.type.rawValue) else { return }
        
        holdedButtons.append(sender.type.rawValue)
        
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            if self.timers[sender.type.rawValue] != nil {
                return
            }
            if self.holdedButtons.contains(sender.type.rawValue) {
    ...
    @objc func buttonUp(_ sender: ExpanderButton) {
        holdedButtons.removeAll { $0 == sender.type.rawValue }
    ...
    func layoutButtons() {
        ...
        guard config != nil && !config.isEmpty else { return }
