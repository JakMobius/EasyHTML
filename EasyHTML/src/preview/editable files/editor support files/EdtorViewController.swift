...
/**
    Первый из трёх контроллеров `EditorController`. Отвечает за показ редактора, основанного на CodeMirror©
 */
...
class EditorViewController: UIViewController, UIScrollViewDelegate, WKScriptMessageHandler, ColorPickerDelegate, GradientPickerDelegate, UIGestureRecognizerDelegate, FindReplaceDialogDelegate, NotificationHandler, EditorPredictiveTextFieldDelegate, ExpanderViewDelegate, WKUIDelegate {
    ...
    // MARK: Переменные фрейма экспандера
    ...
    /// Показывает, активирован ли Emmet конкретно в этой инстанции редактора.
    
    var emmetEnabled = false
    
    /// Показывает, активировано ли автодополнение кода конкретно в этой инстанции редактора.
...
//        KeyCommands.redo,
    ...
    // MARK: Методы, вызывающиеся при нажатии кнопок в дополнительном выдвигающемся окне
    ...
    final func performUndo() {
        ...
        webView.evaluateJavaScript("editor.undo();editor.historySize().undo", completionHandler: {
            ...
            if self.canUndo { // Просто оптимизация
    ...
    private func createFindReplaceDialog(tappedButton button: UIButton, type: FindReplaceDialogContainerView.ButtonActionType, animated: Bool, string: String!) {
        ...
        guard !lockExpanding else { return }
    ...
    func searchInCode(text: String! = nil) {
        guard let button = expanderView.getButton(typed: .search) else {return}
    ...
    func replaceInCode(text: String! = nil) {
        guard let button = expanderView.getButton(typed: .replace) else {return}
    ...
    internal func findReplaceDialog(willClose dialog: FindReplaceDialogContainerView, animated: Bool) {
        ...
        self.expanderView.scrollView.isHidden = false
        if animated {
    ...
    /// Метод, выдвигающий дополнительное окно
    /// - returns: Успешность выполнения запроса

    internal func expand() -> Bool {
        
        guard editorLoaded else { return false }
        guard !lockExpanding else { return false }
        ...
        if isExpanderOpened {
            ...
            self.expanderView.isHidden = false
        } else {
    ...
    @objc func updateExpander() {
        ...
        self.expanderView.config = userPreferences.expanderButtonsList
        self.expanderView.layoutButtons()
    }
...
//        }
    ...
    // MARK: viewDidLoad. В нем производится настройка WKWebView, загрузка страницы, редактора, отображение экрана загрузки, отключение скроллинга
    ...
    //MARK: Слушатели событий колорпикера и градиентпикера
    ...
    final func colorPicker(colorDidChange sender: ColorPicker) {
        guard colorpickerOpened else { return }
        ...
        func sendColor() {
            self.colorPickerNeedsUpdate = false
            self.colorPickerBusy = true
            self.webView.evaluateJavaScript("window.colorpickerCallback('\(sender.colorPreviewText.text ?? "transparent")')", completionHandler: {
                _, _ in
   ...
   final func gradientPicker(didChange sender: GradientPicker) {
    ...
    // MARK: Отслеживание сообщений WKWebView. Служит для показа градиентпикера и колорпикера, отслежку загрузки WKWebView и ввода текста
    ...
    final func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        switch(message.name)
        {
            ...
            case WebViewActions.scrollAction:
                guard let r = message.body as? Array<Int> else { return }
            ...
            case WebViewActions.colorAction:
                ...
                if(!isReadonly && message.body is Array<Any> && (message.body as! Array<Any>).count == 5) {
                    ...
                    if(array[0] is Array<Any>) {
                        ...
                        guard let w = array[3] as? Double else {return}
                        guard let h = array[4] as? Double else {return}
                        ...
                        guard   !x.isNaN &&
                            ...
                            else { print("Wrong color data received!"); return }
        ...
        case WebViewActions.gradientAction:
            ...
            if(array[0] is Array<Any>) {
                ...
                guard w > 0 && h > 0 else { print("Wrong gradient data received!"); return }
                ...
                if(type == 1 || type == 0) {
                    ...
                    for i in 3..<data.count {
                        guard let d = (data[i] as? Array<Any>) else {continue}
        ...
        case WebViewActions.inputAction:
            ...
            guard let r = message.body as? Array<Any> else {return}
            guard
        ...
        if let completionData = data[2] as? Array<Any> {
            ...
            for completionItem in completionData {
                if let textCompletion = completionItem as? String {
                    ...
                    guard let title = complexCompletion[titleKey] else { continue }
    ...
    private func handleJavaScriptScrollAction(offset: CGPoint) {
        if abs(offset.x - self.offsetAfterLastCursorUpdate.x) > cursorUpdateLimit ||
           abs(offset.y - self.offsetAfterLastCursorUpdate.y) > cursorUpdateLimit {
    ...
    private func handleCursor(type: String?, string: String?, completions: [PredictiveTextItem]?, userInfo: NSMutableDictionary?) {
        ...
        if !completions.isEmpty {
            self.predictiveTextView.suggest(items: completions)
        } else {
            if !self.predictiveTextView.predictiveTextField.isEmpty {
                self.predictiveTextView.suggest(items: [])
            }
        }
    }
    ...
    private func updateUndoManager() {
        ...
        guard let firstResponder = webView.firstResponder else { return }
        guard let undoManager = firstResponder.undoManager else { return }
    ...
    private func handleInput(undos: Int, redos: Int, origin: String) {
        self.expanderView.getButton(typed: .undo)?.isEnabled = undos > 0
        self.expanderView.getButton(typed: .redo)?.isEnabled = redos > 0
        
        self.deceleratingVelocity = nil
        
        if(origin != "+replace") {
            self.findReplaceDialog?.updateSearchResults()
        }
    ...
    // MARK: Методы, предназначенные для обработки и загрузки файла в редактор
    ...
    private func getEditorInitalizationScript(data: Data, progress: ((Double) -> ())? = nil, completion: @escaping ((String) -> ())) {
        ...
        DispatchQueue.global().async {
            ...
            if(text == nil) {
                ...
                self.delegate.editor(falledBackToASCII: self)
            ...
            if(userPreferences.emmetEnabled) {
                ...
                let emmetSuppotedExtensions = ["html", "htm", "css"]
                self.emmetEnabled =  emmetSuppotedExtensions.contains(ext)
            } else {
            ...
            if // Если подсветка синтаксиса включена и конфигурация для этого расширения существует
                userPreferences.syntaxHighlightingEnabled,
    ...
    func webViewDidFinishLoad() {
        ...
        DispatchQueue(label: "easyhtml.editor.loadingqueue").async {
            ...
            // Загрузка файлов производится в асинхронном потоке, так, как `getEditorInitializationScript`
            // может работать дольше 10 секунд на старых устройствах
            
            // UPD: Начиная с версии 1.3 загрузка файлов вовсе может происходить с серверов. Так что тем более
            // пихаем это всё в асинхронный поток
            
            /**
                Метод, загружающий файл из сети или с диска, используя указанный `IOManager`
                После загрузки будет вызван метод `getInitScript`
             */
            ...
            /**
                Обработка файла. Получение JS-скрипта, который будет загружен в `WKWebView`.
             - parameter data: Содержимое файла. В процессе будут применены разные кодировки текста
            */
            ...
            /**
                Загрузка редактора и скрипта в `WKWebView`
             - note: Вызывать только в `DispatchQueue.main`
             - parameter script: Скрипт, полученный при помощи функции `getInitScript(data:)`
             */
            
            func loadScript(script: String) {
                
                // Если очень быстро открыть и закрыть редактор,
                // есть шанс того, что редактор успеет
                // деаллокнуться перед тем как вызовется
                // webViewDidFinishLoad.
                // Поэтому во всех отложенных вызовах
                // проверяем его на nil
    ...
    // MARK: Обработка ошибок редактора
    
    /**
     Вызывается в случае ошибки редактора во время загрузки файла
    */
    
    private func editorErrorOccured() {
        guard self.delegate != nil else { return }
        
        messageManager.newWarning(message: localize("editorerror", .editor))
            .applyingStyle(style: .error)
            .withCloseable(false)
            .withButton(EditorWarning.Button(title: localize("send"), target: self, action: #selector(self.sendErrorReport(_:))))
            .withButton(EditorWarning.Button(title: localize("cancel"), target: self, action: #selector(self.closeEditor)))
            .present()
    ...
    /**
     Генерирует отчет об ошибке и показывает окно почтового клиента
    */
    ...
    // MARK: Поддержка бровавых устройств
    ...
    // MARK: Отслеживание изменения размера клавиатуры
    ...
    private func setWebviewBottomInset(_ inset: CGFloat) {
        ...
        bottomInset = inset - (self.tabBarController?.tabBar.frame.height ?? 50)
    ...
    @objc func keyboardWillShow(sender: NSNotification) {
        ...
        if(isReadonly || isKeyboardLocked) {
            ...
            if webView.firstResponder != nil && self.codeAutocompletionEnabled {
    ...
    @objc func keyboardWillChangeFrame(sender: NSNotification) {
        ...
        guard delegate != nil else { return } // Вылеты в AppStore показывают, что оно так умеет.
    ...
    // MARK: Настройка кнопок в дополнительном окне
    ...
    // MARK: Методы PinchGestureRecognizer для динамического изменения размера шрифта
    ...
    @objc func pinchGestureAction(_ sender: UIPinchGestureRecognizer) {
        if sender.state == .began {
            ...
            if sender.numberOfTouches == 2 {
                ...
                // Центрование по оси Y
                // Поскольку JavaScript работает с задержкой и
                // тратит больше ресурсов, мы можем сэкономить
                // энергию, центруя редактор по оси Y при помощи
                // UIKit, а не WKWebView.
    ...
    // MARK: Методы LongPressGestureRecognizer для блокировки скроллинга при выборе текста
    ...
    private func scrollBy(x: CGFloat, y: CGFloat) {
        ...
        DispatchQueue.main.async {
            ...
            /*
                Проверка: Стоит ли делать запрос к WKWebView на изменение отступа скроллинга
 
                Округление до Int сделано из-за того, что JavaScript не позволяет изменять
                позиционирование скроллинга на неполное число. По этой же причине позиция
                скроллинга кэшируется в wkOffset
             */
            ...
            // MARK: Оптимизация в ущерб красоты
            // Экономия 2% - 3% при активном скроллинге
    ...
    // MARK: Методы распознователей жестов для продолжительности скроллинга.
    ...
    /*
         На случай, если я вдруг задумаю новый рефакторинг и буду переписывать весь вью-контроллер с нуля.
         Дружище, НЕ ТРОГАЙ ЭТОТ МЕТОД, Я сам не вдупляю, как оно работает, но оно работает! Просто оставь это
         на месте.
     */
    ...
    private func preventWordSelection() {
        // Кусок кода ниже нужен, чтобы при долгом нажатии и при скроллинге не появлялась
        // лупа и не включалось выделение текста.
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if(preferredContentOffset.y > maxY) { preferredContentOffset.y = maxY }
        if(preferredContentOffset.y < 0) { preferredContentOffset.y = 0 }
    ...
    func showKeyboard() {
        ...
        // Без этой строчки на iPad курсор пропадает после открытия и закрытия spotlight
        
        self.webView.evaluateJavaScript("editor.getInputField().blur();editor.getInputField().focus()", completionHandler: nil)
    }
