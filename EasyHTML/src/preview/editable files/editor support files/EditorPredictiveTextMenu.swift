...
extension UITextChecker {
    static func getSuggestions(for string: String, language: String) -> (suggestions: [String]?, sourceWord: String?) {
        ...
        guard lastWord != nil, lastWordRange != nil else {
            return (nil, nil)
        }
...
class PredictiveTextField: UIScrollView, UIScrollViewDelegate {
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        guard !lockScrollHandling && !subviews.isEmpty else {
            return
        }
        ...
        for view in subviews {
            ...
            guard let view = view as? PredictiveTextItemView else {
                return
            }
    ...
    override func layoutSubviews() {
        ...
        // Gradients work oddly with small coefficients, so introduce a threshold
...
class EditorPredictiveTextMenu: UIVisualEffectView, NotificationHandler {
    ...
    func getKeyboardView() -> UIView? {
        ...
        if #available(iOS 13.0, *) {
            guard let keyboardWindow = UIApplication.shared.windows.first(where: {
                NSStringFromClass(type(of: $0)) == "UIRemoteKeyboardWindow"
            })
            else {
                return nil
            }
            ...
            guard let window = UIApplication.shared.windows.first(where: {
                guard NSStringFromClass(type(of: $0)) == "UITextEffectsWindow" else {
                    return false
                }
                guard let controller = $0.rootViewController else {
                    return false
                }
                guard NSStringFromClass(type(of: controller)) == "UIInputWindowController" else {
                    return false
                }
    ...
    func keyboardWillChangeFrame(sender: NSNotification) {
        if !isHidden {
            becomeVisible()
            predictiveTextField.updateViews()
        }
    }
    ...
    func keyboardWillShow(sender: NSNotification) {
        isHidden = false
        predictiveTextField.subviewsCache.forEach {
            $0.updateColor()
        }
    ...
    private func _becomeVisible() {
        ...
        guard let view = getKeyboardView() else {
            return
        }
