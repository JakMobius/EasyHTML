...
class AppDelegate: UIResponder, UIApplicationDelegate, UISplitViewControllerDelegate, UISceneDelegate {
    ...
    static func receivedFile(at url: URL, window: UIWindow! = nil) {
        ...
        guard let instance = FolderPickerViewController.instance(for: window) else {
            tryAgain(); return
        }
        guard let fileListManager = instance.fileListManager else {
            tryAgain(); return
        }
        guard let split = PrimarySplitViewController.instance(for: window) else {
            tryAgain(); return
        }
        guard let previewContainer = PreviewContainer.instance(for: window) else {
            tryAgain(); return
        }

        // If application was started already, send the file to the FileMovingManager

        let switcherView = PreviewContainer.activeSwitcherView(for: window)

        if (split.isCollapsed) {
            guard let container = split.smallScreenSwitcherView else {
                tryAgain(); return
            }
            guard let first = container.containerViews.first else {
                tryAgain(); return
            }
            container.animateIn(view: first)
        } else {
            guard let container = previewContainer.switcherView else {
                tryAgain(); return
            }
            guard let last = container.lastOpenedView ?? container.containerViews.first else {
                tryAgain(); return
            }
        ...
        guard let file = FSNode.getLocalFile(globalURL: url) else {
            print("[EasyHTML] [AppDelegate] receivedFile(at:) File does not exist!")
            return
        }
